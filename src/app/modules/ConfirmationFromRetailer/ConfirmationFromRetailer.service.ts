...
const updatePendingProductAsRetailerFromDB = async (user: JwtPayload) => {
  ...
  const wholesalerIds = updatedProducts.map((item) =>
    item.wholesaler?.toString()
  );
  const notificationPayload = {
    ...
    } has confirmed the order id please check`,
  };
  await sendNotifications(notificationPayload);
  return updatedProducts;
};

const getAllConfrimRequestFromRetailerIntoDB = async (
  user: JwtPayload,
  query: Record<string, any>
) => {
  const queryBuilder = new QueryBuilder(
    ReplayFromWholesalerModel.find({
      wholesaler: user.id,
      status: "confirmed",
    }),
    query
  ...
  queryBuilder
    .populate(["retailer"], {
      retailer: "name phone storeInformation.location",
    })
    .populate(["wholesaler"], {
      wholesaler: "name phone storeInformation.location",
    })
    .populate(["product"], {
      product:
        "productName unit quantity additionalInfo status createdAt updatedAt",
    });
