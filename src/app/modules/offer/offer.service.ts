...
const updateOfferIntoDB = async (
  ...
  if (
    ...
    productUpdates.some((p) => !mongoose.Types.ObjectId.isValid(p.productId))
  ) {
    throw new ApiError(
      StatusCodes.BAD_REQUEST,
  ...
  const userData = await User.findById(user.id)
    .select("isSubscribed offersUpdatedCount role")
    .lean();
  if (!userData) {
  ...
  const isExistLimit = await OfferModel.countDocuments({
    wholeSeller: user.id,
    status: "Received",
  });
...
const updateOfferFromRetailer = async (
  ...
  try {
    ...
    const existingOffer = await OfferModel.findById(offerId).populate({
