...
const createOfferController = catchAsync(
...
const updateOffer = catchAsync(async (req: Request, res: Response) => {
  ...
  const result = await sendOfferService.updateOfferIntoDB(
    req.user,
    id,
    productUpdates,
    status,
...
const confirmOrderFromRetailer = catchAsync(
  async (req: Request, res: Response) => {
    const { id } = req.params;
    const io = (global as any).io;
...
const getSinglePendingOfferFromRetailer = catchAsync(
  async (req: Request, res: Response) => {
...
const deleteSinglePendingOfferFromRetailer = catchAsync(
  async (req: Request, res: Response) => {
    const user = req.user as JwtPayload;
    if (!user?.id) {
      throw new ApiError(StatusCodes.UNAUTHORIZED, "User is not authenticated");
...
// get single offer from db for retailer
const getSingleReceiveOfferFromRetailerIntoDB = catchAsync(
  async (req: Request, res: Response) => {
    const user = req.user as JwtPayload;
    if (!user?.id) {
      throw new ApiError(StatusCodes.UNAUTHORIZED, "User is not authenticated");
...
const deleteSingleReceiveOfferFromRetailer = catchAsync(
  async (req: Request, res: Response) => {
    const user = req.user as JwtPayload;
    if (!user?.id) {
      throw new ApiError(StatusCodes.UNAUTHORIZED, "User is not authenticated");
...
const deleteSingleConfirmOffer = catchAsync(
  async (req: Request, res: Response) => {
    const user = req.user as JwtPayload;
    if (!user?.id) {
      throw new ApiError(StatusCodes.UNAUTHORIZED, "User is not authenticated");
