...
export const verifyPaymentTransaction = async (
  ...
  try {
    ...
    const response = await axios.get(
      `${FLW_API_URL}/${transaction_id}/verify`,
      {
        headers: {
    ...
    if (
      ...
      const existingPayment = await paymentVerificationModel.findOne({
      ...
      throw new ApiError(
        ...
        "Payment verification failed! Payment status is not successful."
      );
    }
  } catch (error: any) {
    throw new ApiError(
      StatusCodes.BAD_REQUEST,
      `Payment verification failed! Error: ${error.message}`
    );
  }
};
